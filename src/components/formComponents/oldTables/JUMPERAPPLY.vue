    ...
    <div>
         <a-modal v-model="visible" :maskClosable="false" :title="title" on-ok="handleOk" width="40%" :selfUpdate='true' @cancel="handleCancel">
            <a-form-model
                ...
                :rules="rules"
            >
                <a-form-model-item style="margin-bottom:5px"  :wrapper-col="{ span: 16}" label="机房" prop="rsp">
                    <a-select v-model="form.rsp" placeholder="请选择机房" :disabled="updateType==1">
                         <a-select-option
                ...
                <a-form-model-item style="margin-bottom:5px"  :wrapper-col="{ span: 16}" label="服务器" prop="device">
                     <a-select  mode="multiple" v-model="form.device" placeholder="请选择服务器" :disabled="updateType==1">
                         <a-select-option
...
export default {
    ...
    watch:{
        'form.rsp'(newVal,oldVal){
            ...
            this.getSelectData({rsp: rsp,}, "/v2/ajax/cmdb/listdevices",'device')
        },
    },
    methods:{
